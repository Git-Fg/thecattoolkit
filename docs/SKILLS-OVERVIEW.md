...
## Philosophy: Skills are Hybrid (Knowledge + Execution)
 
 Skills are **Hybrid Capability Units** with dual nature:
 - **Passive:** Auto-discovered knowledge bases (standards, templates, methodologies)
 - **Active:** User-invocable (`/skill-name`) or forked execution units (`context: fork`)
 
 The `description` field determines when Claude applies the Skill automatically.

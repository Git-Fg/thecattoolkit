---
name: test-writer
description: "MUST USE when generating comprehensive Vitest tests for code examples in JavaScript concept documentation pages. Follows project conventions, extracts examples from documentation, converts them to tests, and ensures documentation accuracy through automated testing."
user-invocable: true
...
1. **Scan the documentation** for all code examples
2. **Categorize examples** by type (testable, DOM, error, conceptual)
3. **Determine appropriate test structure** based on content
4. **Generate comprehensive tests** following the patterns above
5. **Create both standard and DOM test files** when needed
6. **Include source line references** for traceability
7. **Verify test file placement** follows project conventions

**Remember:** Tests serve two purposes:
1. Verify documentation is accurate
2. Catch regressions if code examples are updated

Every testable code example in the documentation should have a corresponding test. If an example can't be tested, document why in the test file.

...
cd your-project
cp -r /path/to/cat-toolkit/plugins/guard/hooks/scripts .cattoolkit/hooks/
```
...
### Hook Timeout
- Hooks have 10-30 second timeouts
...
For hooks to function, they must be deployed to the project's `.cattoolkit/hooks/` directory:

```bash
# Create hooks directory
mkdir -p .cattoolkit/hooks

# Copy hooks configuration
cp plugins/guard/hooks/hooks.json .cattoolkit/hooks/

# Verify deployment
cat .cattoolkit/hooks/hooks.json
```

The hook_runner.py will automatically check for `.cattoolkit/hooks/` and skip silently if not deployed.

...
def try_command(cmd: list[str], timeout: int = 2) -> bool:
...
def get_type_checker_command(
    checker: TypeChecker, project_root: Path
    ...
    if checker == "pyrefly":
        ...
        if check_command_available("uv"):
            if try_command(["uv", "run", "pyrefly", "--version"], timeout=2):
    ...
    if checker == "mypy":
        ...
        if check_command_available("uv"):
            if try_command(["uv", "run", "mypy", "--version"], timeout=2):
...
def main() -> None:
    try:
        ...
        try:
            result = subprocess.run(full_cmd, capture_output=True, timeout=2)

...
**When it fires**: Permission dialog is shown to the user
...
```json
{
  "hookSpecificOutput": {
    ...
    "decision": {
      ...
      "message": "Why permission was denied",
      "interrupt": true
    }
  }
}
...
- Provide custom permission feedback
    ...
    "PermissionRequest": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python3 ${CLAUDE_PLUGIN_ROOT}/.claude/hooks/scripts/auto-approve.py"
          }
        ]
      }
    ]
...
**When it fires**: A subagent starts execution
...
- Log subagent initialization
- Set up subagent context
- Initialize subagent-specific state
- Notify external systems

**Example**: Log subagent starts
    ...
    "SubagentStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "echo \"Subagent started: $(date)\" >> ~/.claude/subagent-log.txt"
          }
        ]
      }
    ]

...
def test_hook_script(script_path: str, test_input: str = None, verbose: bool = False) -> Tuple[bool, str]:
    """
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("\nUsage:")
        print("  python3 hook-tester.py validate <hooks.json>       - Validate hooks.json structure")
        print("  python3 hook-tester.py test <hooks.json>           - Test all hook scripts")
        print("  python3 hook-tester.py security-check <hooks.json> - Check security patterns")
        print("  python3 hook-tester.py debug <hooks.json>          - Detailed diagnostic output")
        print("  python3 hook-tester.py permissions <hooks.json>    - Check execute permissions")
        print("  python3 hook-tester.py json-check <hooks.json>      - Validate JSON output structure")
        print("  python3 hook-tester.py quick-diag <hooks.json>     - Run all checks at once")
        print("\nExamples:")
    ...
    config = None
    if command in ['validate', 'test', 'security-check', 'debug', 'permissions', 'json-check', 'quick-diag']:
        try:
            with open(config_path, 'r') as f:
                config = json.load(f)
        except FileNotFoundError:
    ...
    if command == 'validate':
            ...
            for hook_config in hook_list:
                for hook in hook_config.get('hooks', []):
                    if hook.get('type') == 'command':
                        ...
                        success, output = test_hook_script(script_path, test_input, verbose=True)
                ...
                for hook in hook_config.get('hooks', []):
                    if hook.get('type') == 'command':
                        ...
                        if success:
                            ...
                            print(f"  ✗ {script_path} - Script failed (can't validate JSON)")
            print()
        ...
        # Summary
        print("=== SUMMARY ===")
        if success and perm_issues == 0 and scripts_passed == scripts_tested and json_issues == 0 and security_issues == 0:
        ...
        print(f"Available commands: validate, test, security-check, debug, permissions, json-check, quick-diag")
        sys.exit(1)

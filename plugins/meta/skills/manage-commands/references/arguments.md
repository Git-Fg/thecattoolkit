...
## Default Values with Shell Syntax

**Source**: Shell parameter expansion pattern

Use shell-style default values for optional arguments:

```markdown
---
description: Analyze code or use current directory
argument-hint: [path]
---

Analyze the code at: ${ARGUMENTS:-.}

If no path provided, analyzes the current directory (.)
```

**Usage 1**: `/analyze src/app.js` → analyzes `src/app.js`
**Usage 2**: `/analyze` → analyzes `.` (current directory)

**Pattern**: `${ARGUMENTS:-default_value}`

This shell parameter expansion syntax provides default values when arguments are empty.

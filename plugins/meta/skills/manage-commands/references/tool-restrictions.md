...
**Default**: If omitted, **inherits tools from the conversation**

**CRITICAL**: `allowed-tools` is for **restricting** tools, NOT enabling them. Commands automatically have access to all tools available in the current conversation context. Only use `allowed-tools` when you need to **limit** what a command can do.

### Default Behavior (No allowed-tools)
...
**Result**: Command inherits ALL tools from the current conversation. If the conversation has access to `Task`, `Bash`, `Read`, `Write`, etc., the command can use them all.

### When to Omit allowed-tools

Most commands should **NOT** specify `allowed-tools`:

- General-purpose commands that need flexibility
- Commands that use agents (inherit Task tool through conversation)
- Commands that may need various tools depending on context
- When you want full conversation inheritance
...
**CRITICAL**: Commands that delegate to agents should **NOT** have restrictive `allowed-tools` settings. Agents should have access to all tools through conversation inheritance.

### Correct Pattern (No allowed-tools)

```yaml
---
description: Audit an agent for best practices
argument-hint: [agent-name-or-path]
---

## Objective
Audit an agent configuration file.

## Process
Use plugin-expert to audit:

```
Audit the agent at: ${ARGUMENTS:-.}
```
```

**Result**: Command inherits all tools from conversation, can delegate to agents.

### Incorrect Pattern (Overly Restricted)

```yaml
---
description: Audit an agent
allowed-tools: [Task]  # ❌ WRONG - prevents other needed tools
argument-hint: [agent-name-or-path]
---
```

**Result**: Command can ONLY use `Task`, cannot use `Read`, `Grep`, `Glob` for file discovery.

### When to Explicitly Restrict Tools

Only use restrictive `allowed-tools` when:
- Command is a simple delegation gateway
- You want to ensure ONLY delegation happens, no direct operations
- Combined with other specific tools the command needs

```yaml
---
description: Simple delegation command
allowed-tools: [Task, Read, AskUserQuestion]
argument-hint: [request]
---

Get user input, then delegate to agent.
```

## Bash Command Requirement

**Source**: Official Claude Code documentation

When using bash commands with the `!` prefix for dynamic context loading, you **must** include `allowed-tools` with the `Bash` tool:
...
**Rule**: If command uses `!` prefix, must specify `allowed-tools: Bash(...)` pattern.

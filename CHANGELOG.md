...
#### AI Rule Synchronization Command

- [`/uprules`](./commands/uprules.md) - Audit and update AI rule files to synchronize them with code changes through systematic interview
  - Analyzes agent configurations (CLAUDE.md, AGENTS.md, GEMINI.md.claude, .agent, .gemini)
  - Identifies discrepancies between rules and actual implementation
  - Provides informed suggestions for improvement, synchronization, and completion
  - Updates rule files to create comprehensive source of truth

### Benefits

#### Automated Rule Maintenance

AI agent configuration files automatically reflect project requirements without manual oversight

#### Proactive Rule Synchronization

Identifies and resolves discrepancies between code reality and agent rules before they cause issues
...
- Removed `format-on-edit.py` hook and configuration
- Removed all documentation references to auto-formatting
- Cleaned up hook configuration in `hooks.json`

### Benefits

#### Simplified Workflow

- Reduced unexpected file modifications during editing
- Streamlined hook execution pipeline
